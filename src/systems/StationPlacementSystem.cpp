...
StationPlacementSystem::StationPlacementSystem(ServiceLocator& serviceLocator)
    ...
    m_mousePressConnection = serviceLocator.eventBus->sink<MouseButtonPressedEvent>().connect<&StationPlacementSystem::onMouseButtonPressed>(this);
...
void StationPlacementSystem::onMouseButtonPressed(const MouseButtonPressedEvent& event) {
    ...
    if (_gameState->currentInteractionMode == InteractionMode::CREATE_STATION && event.button == sf::Mouse::Button::Right) {
        LOG_DEBUG("StationPlacementSystem", "Processing MouseButtonPressedEvent at (%.1f, %.1f)", event.worldPosition.x, event.worldPosition.y);
