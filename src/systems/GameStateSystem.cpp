...
GameStateSystem::GameStateSystem(ServiceLocator& services)
    : _services(services) {
    _interactionModeChangeListener = _services.eventBus->sink<InteractionModeChangeEvent>().connect<&GameStateSystem::onInteractionModeChange>(this);
    LOG_INFO("GameStateSystem", "GameStateSystem created and listening for interaction mode changes.");
}
