...
CameraSystem::CameraSystem(ServiceLocator& serviceLocator)
    : m_camera(serviceLocator.camera),
      m_window(&serviceLocator.renderer->getWindowInstance()) {
    m_zoomConnection = serviceLocator.eventBus->sink<CameraZoomEvent>().connect<&CameraSystem::onCameraZoom>(this);
    m_panConnection = serviceLocator.eventBus->sink<CameraPanEvent>().connect<&CameraSystem::onCameraPan>(this);
